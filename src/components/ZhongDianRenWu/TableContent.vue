<template>
  <div class="table--content--container">
    ...
    <table v-if="currentArray.length > 0">
      ...
      <tr v-for="i in (pageNumber-currentArray.length)" :key="'spacer'+i" style="background: transparent">
        <td></td>
      </tr>
    </table>
...
export default {
  ...
  computed: {
    pages () {
      return Math.ceil(this.processedArray.length / this.pageNumber) || 1
    }
  },
  mounted () {
  ...
  methods: {
    ...
    onChangePage (p) {
      if (!p) return
      this.currentPage = p
      this.currentArray = this.processedArray.slice((p - 1) * this.pageNumber, (p - 1) * this.pageNumber + this.pageNumber)
    }
  }
}
</script>
